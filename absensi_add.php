...
$layout->blocks["top"][] = "details";$page_layouts["absensi_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "absensi_inline_add.htm";
else
	$templatefile = "absensi_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: absensi_add.php');
...
//	processing nip - end
//	processing tanggal_absen - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_tanggal_absen = $pageObject->getControl("tanggal_absen", $id);
		$control_tanggal_absen->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tanggal_absen - end
//	processing jam_masuk - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jam_masuk = $pageObject->getControl("jam_masuk", $id);
		$control_jam_masuk->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing jam_masuk - end
//	processing jam_keluar - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jam_keluar = $pageObject->getControl("jam_keluar", $id);
		$control_jam_keluar->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing jam_keluar - end
//	processing status_masuk - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_status_masuk = $pageObject->getControl("status_masuk", $id);
		$control_status_masuk->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing status_masuk - end
//	processing status_keluar - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_status_keluar = $pageObject->getControl("status_keluar", $id);
		$control_status_keluar->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing status_keluar - end
//	processing ket - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ket = $pageObject->getControl("ket", $id);
		$control_ket->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ket - end
//	processing terlambat - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_terlambat = $pageObject->getControl("terlambat", $id);
		$control_terlambat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'absensi_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'absensi_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: absensi_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_absensi"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_absensi"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["tanggal_absen"]=@$avalues["tanggal_absen"];
	$defvalues["jam_masuk"]=@$avalues["jam_masuk"];
	$defvalues["jam_keluar"]=@$avalues["jam_keluar"];
	$defvalues["status_masuk"]=@$avalues["status_masuk"];
	$defvalues["status_keluar"]=@$avalues["status_keluar"];
	$defvalues["ket"]=@$avalues["ket"];
	$defvalues["terlambat"]=@$avalues["terlambat"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tanggal_absen_label","<label for=\"".GetInputElementId("tanggal_absen", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jam_masuk_label","<label for=\"".GetInputElementId("jam_masuk", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jam_keluar_label","<label for=\"".GetInputElementId("jam_keluar", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("status_masuk_label","<label for=\"".GetInputElementId("status_masuk", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("status_keluar_label","<label for=\"".GetInputElementId("status_keluar", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ket_label","<label for=\"".GetInputElementId("ket", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("terlambat_label","<label for=\"".GetInputElementId("terlambat", $id, PAGE_ADD)."\">","</label>");
