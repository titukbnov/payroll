...
$layout->blocks["top"][] = "details";$page_layouts["pelatihan_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pelatihan_inline_add.htm";
else
	$templatefile = "pelatihan_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pelatihan_add.php');
...
//	processing nip - end
//	processing tgl_pelatihan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_tgl_pelatihan = $pageObject->getControl("tgl_pelatihan", $id);
		$control_tgl_pelatihan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tgl_pelatihan - end
//	processing sk_pelatihan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sk_pelatihan = $pageObject->getControl("sk_pelatihan", $id);
		$control_sk_pelatihan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sk_pelatihan - end
//	processing topik_pelatihan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_topik_pelatihan = $pageObject->getControl("topik_pelatihan", $id);
		$control_topik_pelatihan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing topik_pelatihan - end
//	processing penyelenggara - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_penyelenggara = $pageObject->getControl("penyelenggara", $id);
		$control_penyelenggara->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing penyelenggara - end
//	processing hasil_pelatihan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_hasil_pelatihan = $pageObject->getControl("hasil_pelatihan", $id);
		$control_hasil_pelatihan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pelatihan_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pelatihan_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pelatihan_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_pelatihan"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_pelatihan"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["tgl_pelatihan"]=@$avalues["tgl_pelatihan"];
	$defvalues["sk_pelatihan"]=@$avalues["sk_pelatihan"];
	$defvalues["topik_pelatihan"]=@$avalues["topik_pelatihan"];
	$defvalues["penyelenggara"]=@$avalues["penyelenggara"];
	$defvalues["hasil_pelatihan"]=@$avalues["hasil_pelatihan"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tgl_pelatihan_label","<label for=\"".GetInputElementId("tgl_pelatihan", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sk_pelatihan_label","<label for=\"".GetInputElementId("sk_pelatihan", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("topik_pelatihan_label","<label for=\"".GetInputElementId("topik_pelatihan", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("penyelenggara_label","<label for=\"".GetInputElementId("penyelenggara", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("hasil_pelatihan_label","<label for=\"".GetInputElementId("hasil_pelatihan", $id, PAGE_ADD)."\">","</label>");
