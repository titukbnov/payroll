...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("user_level"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("user_level")] = GetTableURL($pageObject->pSet->getLookupTable("user_level"));
	
	$pageObject->fillFieldToolTips("user_level");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("user_level");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "user_level";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("user_level_label","<label for=\"".GetInputElementId("user_level", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("user_level_label", true);
	
	$xt->assign("user_level_fieldblock", true);
	$xt->assignbyref("user_level_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("user_level_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("user_level_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_user_level", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("user_level");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"user_level", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"user_level", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("last_update"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("last_update")] = GetTableURL($pageObject->pSet->getLookupTable("last_update"));
	
	$pageObject->fillFieldToolTips("last_update");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("last_update");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "last_update";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("last_update_label","<label for=\"".GetInputElementId("last_update", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("last_update_label", true);
	
	$xt->assign("last_update_fieldblock", true);
	$xt->assignbyref("last_update_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("last_update_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("last_update_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_last_update", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("last_update");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"last_update", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"last_update", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
