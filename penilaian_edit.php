...
$layout->blocks["top"][] = "details";$page_layouts["penilaian_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "penilaian_inline_edit.htm" : "penilaian_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["id_penilaian"] = urldecode(postvalue("editid1"));
$savedKeys["id_penilaian"] = urldecode(postvalue("editid1"));
...
//	processing nip - end
//	processing sk_penilaian - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sk_penilaian = $pageObject->getControl("sk_penilaian", $id);
		$control_sk_penilaian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sk_penilaian - end
//	processing periode_penilaian - begin
	$condition = 1;

	if($condition)
	{
		$control_periode_penilaian = $pageObject->getControl("periode_penilaian", $id);
		$control_periode_penilaian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing periode_penilaian - end
//	processing judul_penilaian - begin
	$condition = 1;

	if($condition)
	{
		$control_judul_penilaian = $pageObject->getControl("judul_penilaian", $id);
		$control_judul_penilaian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing judul_penilaian - end
//	processing indikator_a - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_indikator_a = $pageObject->getControl("indikator_a", $id);
		$control_indikator_a->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing indikator_a - end
//	processing indikator_b - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_indikator_b = $pageObject->getControl("indikator_b", $id);
		$control_indikator_b->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing indikator_b - end
//	processing indikator_c - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_indikator_c = $pageObject->getControl("indikator_c", $id);
		$control_indikator_c->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing indikator_c - end
//	processing indikator_d - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_indikator_d = $pageObject->getControl("indikator_d", $id);
		$control_indikator_d->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing indikator_d - end
//	processing indikator_e - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_indikator_e = $pageObject->getControl("indikator_e", $id);
		$control_indikator_e->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing sasaran - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sasaran = $pageObject->getControl("sasaran", $id);
		$control_sasaran->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sasaran - end
//	processing pencapaian - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_pencapaian = $pageObject->getControl("pencapaian", $id);
		$control_pencapaian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pencapaian - end
//	processing hasil_penilaian - begin
	$condition = 1;

	if($condition)
	{
		$control_hasil_penilaian = $pageObject->getControl("hasil_penilaian", $id);
		$control_hasil_penilaian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_penilaian"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: penilaian_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["sk_penilaian"] = $evalues["sk_penilaian"];
	$data["periode_penilaian"] = $evalues["periode_penilaian"];
	$data["judul_penilaian"] = $evalues["judul_penilaian"];
	$data["indikator_a"] = $evalues["indikator_a"];
	$data["indikator_b"] = $evalues["indikator_b"];
	$data["indikator_c"] = $evalues["indikator_c"];
	$data["indikator_d"] = $evalues["indikator_d"];
	$data["indikator_e"] = $evalues["indikator_e"];
	$data["satuan"] = $evalues["satuan"];
	$data["sasaran"] = $evalues["sasaran"];
	$data["pencapaian"] = $evalues["pencapaian"];
	$data["hasil_penilaian"] = $evalues["hasil_penilaian"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("sk_penilaian_label","<label for=\"".GetInputElementId("sk_penilaian", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("periode_penilaian_label","<label for=\"".GetInputElementId("periode_penilaian", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("judul_penilaian_label","<label for=\"".GetInputElementId("judul_penilaian", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("indikator_a_label","<label for=\"".GetInputElementId("indikator_a", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("indikator_b_label","<label for=\"".GetInputElementId("indikator_b", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("indikator_c_label","<label for=\"".GetInputElementId("indikator_c", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("indikator_d_label","<label for=\"".GetInputElementId("indikator_d", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("indikator_e_label","<label for=\"".GetInputElementId("indikator_e", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sasaran_label","<label for=\"".GetInputElementId("sasaran", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pencapaian_label","<label for=\"".GetInputElementId("pencapaian", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("hasil_penilaian_label","<label for=\"".GetInputElementId("hasil_penilaian", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='penilaian_view.php?".$viewlink."'\"");
