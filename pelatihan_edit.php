...
$layout->blocks["top"][] = "details";$page_layouts["pelatihan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pelatihan_inline_edit.htm" : "pelatihan_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["id_pelatihan"] = urldecode(postvalue("editid1"));
$savedKeys["id_pelatihan"] = urldecode(postvalue("editid1"));
...
//	processing nip - end
//	processing tgl_pelatihan - begin
	$condition = 1;

	if($condition)
	{
		$control_tgl_pelatihan = $pageObject->getControl("tgl_pelatihan", $id);
		$control_tgl_pelatihan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tgl_pelatihan - end
//	processing sk_pelatihan - begin
	$condition = 1;

	if($condition)
	{
		$control_sk_pelatihan = $pageObject->getControl("sk_pelatihan", $id);
		$control_sk_pelatihan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sk_pelatihan - end
//	processing topik_pelatihan - begin
	$condition = 1;

	if($condition)
	{
		$control_topik_pelatihan = $pageObject->getControl("topik_pelatihan", $id);
		$control_topik_pelatihan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing topik_pelatihan - end
//	processing penyelenggara - begin
	$condition = 1;

	if($condition)
	{
		$control_penyelenggara = $pageObject->getControl("penyelenggara", $id);
		$control_penyelenggara->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing penyelenggara - end
//	processing hasil_pelatihan - begin
	$condition = 1;

	if($condition)
	{
		$control_hasil_pelatihan = $pageObject->getControl("hasil_pelatihan", $id);
		$control_hasil_pelatihan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing hasil_pelatihan - end
	...
	//	processing nip - end
			//	processing tgl_pelatihan - begin
				...
				if($condition)
				{
					$control_tgl_pelatihan->afterSuccessfulSave();
				}
	//	processing tgl_pelatihan - end
			//	processing sk_pelatihan - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_sk_pelatihan->afterSuccessfulSave();
				}
	//	processing sk_pelatihan - end
			//	processing topik_pelatihan - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_topik_pelatihan->afterSuccessfulSave();
				}
	//	processing topik_pelatihan - end
			//	processing penyelenggara - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_penyelenggara->afterSuccessfulSave();
				}
	//	processing penyelenggara - end
			//	processing hasil_pelatihan - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_hasil_pelatihan->afterSuccessfulSave();
				}
	//	processing hasil_pelatihan - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_pelatihan"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: pelatihan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pelatihan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["tgl_pelatihan"] = $evalues["tgl_pelatihan"];
	$data["sk_pelatihan"] = $evalues["sk_pelatihan"];
	$data["topik_pelatihan"] = $evalues["topik_pelatihan"];
	$data["penyelenggara"] = $evalues["penyelenggara"];
	$data["hasil_pelatihan"] = $evalues["hasil_pelatihan"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("tgl_pelatihan"))
		$xt->assign("tgl_pelatihan_fieldblock",true);
	else
		$xt->assign("tgl_pelatihan_tabfieldblock",true);
	$xt->assign("tgl_pelatihan_label",true);
	if(isEnableSection508())
		$xt->assign_section("tgl_pelatihan_label","<label for=\"".GetInputElementId("tgl_pelatihan", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sk_pelatihan"))
		$xt->assign("sk_pelatihan_fieldblock",true);
	else
		$xt->assign("sk_pelatihan_tabfieldblock",true);
	$xt->assign("sk_pelatihan_label",true);
	if(isEnableSection508())
		$xt->assign_section("sk_pelatihan_label","<label for=\"".GetInputElementId("sk_pelatihan", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("topik_pelatihan"))
		$xt->assign("topik_pelatihan_fieldblock",true);
	else
		$xt->assign("topik_pelatihan_tabfieldblock",true);
	$xt->assign("topik_pelatihan_label",true);
	if(isEnableSection508())
		$xt->assign_section("topik_pelatihan_label","<label for=\"".GetInputElementId("topik_pelatihan", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("penyelenggara"))
		$xt->assign("penyelenggara_fieldblock",true);
	else
		$xt->assign("penyelenggara_tabfieldblock",true);
	$xt->assign("penyelenggara_label",true);
	if(isEnableSection508())
		$xt->assign_section("penyelenggara_label","<label for=\"".GetInputElementId("penyelenggara", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("hasil_pelatihan"))
		$xt->assign("hasil_pelatihan_fieldblock",true);
	else
		$xt->assign("hasil_pelatihan_tabfieldblock",true);
	$xt->assign("hasil_pelatihan_label",true);
	if(isEnableSection508())
		$xt->assign_section("hasil_pelatihan_label","<label for=\"".GetInputElementId("hasil_pelatihan", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id_pelatihan", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["id_pelatihan"]));
...
//	tgl_pelatihan - Short Date
	$value = $pageObject->showDBValue("tgl_pelatihan", $data, $keylink);
	$showValues["tgl_pelatihan"] = $value;
	$showFields[] = "tgl_pelatihan";
		$showRawValues["tgl_pelatihan"] = substr($data["tgl_pelatihan"],0,100);

//	sk_pelatihan - 
	$value = $pageObject->showDBValue("sk_pelatihan", $data, $keylink);
	$showValues["sk_pelatihan"] = $value;
	$showFields[] = "sk_pelatihan";
		$showRawValues["sk_pelatihan"] = substr($data["sk_pelatihan"],0,100);

//	topik_pelatihan - 
	$value = $pageObject->showDBValue("topik_pelatihan", $data, $keylink);
	$showValues["topik_pelatihan"] = $value;
	$showFields[] = "topik_pelatihan";
		$showRawValues["topik_pelatihan"] = substr($data["topik_pelatihan"],0,100);

//	penyelenggara - 
	$value = $pageObject->showDBValue("penyelenggara", $data, $keylink);
	$showValues["penyelenggara"] = $value;
	$showFields[] = "penyelenggara";
		$showRawValues["penyelenggara"] = substr($data["penyelenggara"],0,100);

//	hasil_pelatihan - 
	$value = $pageObject->showDBValue("hasil_pelatihan", $data, $keylink);
	$showValues["hasil_pelatihan"] = $value;
	$showFields[] = "hasil_pelatihan";
		$showRawValues["hasil_pelatihan"] = substr($data["hasil_pelatihan"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pelatihan";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "pelatihan";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pelatihan_view.php?".$viewlink."'\"");
