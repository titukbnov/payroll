...
$layout->blocks["top"][] = "details";$page_layouts["login_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "login_inline_add.htm";
else
	$templatefile = "login_add.htm";
...
//	processing nip - end
//	processing user_level - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_user_level = $pageObject->getControl("user_level", $id);
		$control_user_level->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing last_update - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_last_update = $pageObject->getControl("last_update", $id);
		$control_last_update->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'login_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'login_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: login_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_login"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_login"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["user_level"]=@$avalues["user_level"];
	$defvalues["last_login"]=@$avalues["last_login"];
	$defvalues["last_update"]=@$avalues["last_update"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("user_level_label","<label for=\"".GetInputElementId("user_level", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("last_update_label","<label for=\"".GetInputElementId("last_update", $id, PAGE_ADD)."\">","</label>");
