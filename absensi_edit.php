...
$layout->blocks["top"][] = "details";$page_layouts["absensi_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "absensi_inline_edit.htm" : "absensi_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["id_absensi"] = urldecode(postvalue("editid1"));
$savedKeys["id_absensi"] = urldecode(postvalue("editid1"));
...
//	processing nip - end
//	processing tanggal_absen - begin
	$condition = 1;

	if($condition)
	{
		$control_tanggal_absen = $pageObject->getControl("tanggal_absen", $id);
		$control_tanggal_absen->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tanggal_absen - end
//	processing jam_masuk - begin
	$condition = 1;

	if($condition)
	{
		$control_jam_masuk = $pageObject->getControl("jam_masuk", $id);
		$control_jam_masuk->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jam_masuk - end
//	processing jam_keluar - begin
	$condition = 1;

	if($condition)
	{
		$control_jam_keluar = $pageObject->getControl("jam_keluar", $id);
		$control_jam_keluar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jam_keluar - end
//	processing status_masuk - begin
	$condition = 1;

	if($condition)
	{
		$control_status_masuk = $pageObject->getControl("status_masuk", $id);
		$control_status_masuk->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing status_masuk - end
//	processing status_keluar - begin
	$condition = 1;

	if($condition)
	{
		$control_status_keluar = $pageObject->getControl("status_keluar", $id);
		$control_status_keluar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing status_keluar - end
//	processing ket - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_ket = $pageObject->getControl("ket", $id);
		$control_ket->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ket - end
//	processing terlambat - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_terlambat = $pageObject->getControl("terlambat", $id);
		$control_terlambat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing terlambat - end
	...
	//	processing nip - end
			//	processing tanggal_absen - begin
				...
				if($condition)
				{
					$control_tanggal_absen->afterSuccessfulSave();
				}
	//	processing tanggal_absen - end
			//	processing jam_masuk - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_jam_masuk->afterSuccessfulSave();
				}
	//	processing jam_masuk - end
			//	processing jam_keluar - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_jam_keluar->afterSuccessfulSave();
				}
	//	processing jam_keluar - end
			//	processing status_masuk - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_status_masuk->afterSuccessfulSave();
				}
	//	processing status_masuk - end
			//	processing status_keluar - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_status_keluar->afterSuccessfulSave();
				}
	//	processing status_keluar - end
			//	processing ket - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_ket->afterSuccessfulSave();
				}
	//	processing ket - end
			//	processing terlambat - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_terlambat->afterSuccessfulSave();
				}
	//	processing terlambat - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_absensi"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: absensi_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["tanggal_absen"] = $evalues["tanggal_absen"];
	$data["jam_masuk"] = $evalues["jam_masuk"];
	$data["jam_keluar"] = $evalues["jam_keluar"];
	$data["status_masuk"] = $evalues["status_masuk"];
	$data["status_keluar"] = $evalues["status_keluar"];
	$data["ket"] = $evalues["ket"];
	$data["terlambat"] = $evalues["terlambat"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tanggal_absen_label","<label for=\"".GetInputElementId("tanggal_absen", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jam_masuk_label","<label for=\"".GetInputElementId("jam_masuk", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jam_keluar_label","<label for=\"".GetInputElementId("jam_keluar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("status_masuk_label","<label for=\"".GetInputElementId("status_masuk", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("status_keluar_label","<label for=\"".GetInputElementId("status_keluar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ket_label","<label for=\"".GetInputElementId("ket", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("terlambat_label","<label for=\"".GetInputElementId("terlambat", $id, PAGE_EDIT)."\">","</label>");
...
//	tanggal_absen - Short Date
	...
	$showValues["tanggal_absen"] = $value;
	$showFields[] = "tanggal_absen";
		$showRawValues["tanggal_absen"] = substr($data["tanggal_absen"],0,100);

//	jam_masuk - Time
	$value = $pageObject->showDBValue("jam_masuk", $data, $keylink);
	$showValues["jam_masuk"] = $value;
	$showFields[] = "jam_masuk";
		$showRawValues["jam_masuk"] = substr($data["jam_masuk"],0,100);

//	jam_keluar - Time
	$value = $pageObject->showDBValue("jam_keluar", $data, $keylink);
	$showValues["jam_keluar"] = $value;
	$showFields[] = "jam_keluar";
		$showRawValues["jam_keluar"] = substr($data["jam_keluar"],0,100);

//	status_masuk - 
	$value = $pageObject->showDBValue("status_masuk", $data, $keylink);
	$showValues["status_masuk"] = $value;
	$showFields[] = "status_masuk";
		$showRawValues["status_masuk"] = substr($data["status_masuk"],0,100);

//	status_keluar - 
	$value = $pageObject->showDBValue("status_keluar", $data, $keylink);
	$showValues["status_keluar"] = $value;
	$showFields[] = "status_keluar";
		$showRawValues["status_keluar"] = substr($data["status_keluar"],0,100);

//	ket - 
	$value = $pageObject->showDBValue("ket", $data, $keylink);
	$showValues["ket"] = $value;
	$showFields[] = "ket";
		$showRawValues["ket"] = substr($data["ket"],0,100);

//	terlambat - 
	$value = $pageObject->showDBValue("terlambat", $data, $keylink);
	$showValues["terlambat"] = $value;
	$showFields[] = "terlambat";
		$showRawValues["terlambat"] = substr($data["terlambat"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='absensi_view.php?".$viewlink."'\"");
