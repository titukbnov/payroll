...
include("include/login_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["login_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "login_inline_edit.htm" : "login_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["id_login"] = urldecode(postvalue("editid1"));
$savedKeys["id_login"] = urldecode(postvalue("editid1"));
...
//	processing nip - end
//	processing user_level - begin
	$condition = 1;

	if($condition)
	{
		$control_user_level = $pageObject->getControl("user_level", $id);
		$control_user_level->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing user_level - end
...
//	processing last_update - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_last_update = $pageObject->getControl("last_update", $id);
		$control_last_update->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing last_update - end
...
//	processing created - end
	...
	//	processing nip - end
			//	processing user_level - begin
				...
				if($condition)
				{
					$control_user_level->afterSuccessfulSave();
				}
	//	processing user_level - end
			//	processing last_login - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing last_login - end
			//	processing last_update - begin
				...
				if($condition)
				{
					$control_last_update->afterSuccessfulSave();
				}
	//	processing last_update - end
			//	processing created - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_login"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: login_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: login_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["user_level"] = $evalues["user_level"];
	$data["last_login"] = $evalues["last_login"];
	$data["last_update"] = $evalues["last_update"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("user_level"))
		$xt->assign("user_level_fieldblock",true);
	else
		$xt->assign("user_level_tabfieldblock",true);
	$xt->assign("user_level_label",true);
	if(isEnableSection508())
		$xt->assign_section("user_level_label","<label for=\"".GetInputElementId("user_level", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("last_update"))
		$xt->assign("last_update_fieldblock",true);
	else
		$xt->assign("last_update_tabfieldblock",true);
	$xt->assign("last_update_label",true);
	if(isEnableSection508())
		$xt->assign_section("last_update_label","<label for=\"".GetInputElementId("last_update", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id_login", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["id_login"]));
...
//	user_level - 
	$value = $pageObject->showDBValue("user_level", $data, $keylink);
	$showValues["user_level"] = $value;
	$showFields[] = "user_level";
		$showRawValues["user_level"] = substr($data["user_level"],0,100);
...
//	last_update - Short Date
	$value = $pageObject->showDBValue("last_update", $data, $keylink);
	$showValues["last_update"] = $value;
	$showFields[] = "last_update";
		$showRawValues["last_update"] = substr($data["last_update"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='login_view.php?".$viewlink."'\"");
