...
$layout->blocks["top"][] = "export";$page_layouts["login_export"] = $layout;
...
function ExportToXML($cipherer)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["user_level"] = $pageObject->showDBValue("user_level", $row);
			$values["last_login"] = $pageObject->showDBValue("last_login", $row);
			$values["last_update"] = $pageObject->showDBValue("last_update", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["user_level"] = $pageObject->getViewControl("user_level")->showDBValue($row, "");
			$values["last_login"] = $pageObject->getViewControl("last_login")->showDBValue($row, "");
			$values["last_update"] = $pageObject->getViewControl("last_update")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["user_level"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["last_login"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["last_update"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("User Level").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Last Login").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Last Update").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["user_level"]);
					else
						echo $values["user_level"];
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["last_login"]);
					else
						echo $values["last_login"];
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["last_update"]);
					else
						echo $values["last_update"];
			echo '</td>';
							echo '<td>';
