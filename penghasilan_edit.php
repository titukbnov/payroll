...
$layout->blocks["top"][] = "details";$page_layouts["penghasilan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "penghasilan_inline_edit.htm" : "penghasilan_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["id_penghasilan"] = urldecode(postvalue("editid1"));
$savedKeys["id_penghasilan"] = urldecode(postvalue("editid1"));
...
//	processing nip - end
//	processing gaji_pokok - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_gaji_pokok = $pageObject->getControl("gaji_pokok", $id);
		$control_gaji_pokok->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing gaji_pokok - end
//	processing tunjangan - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tunjangan = $pageObject->getControl("tunjangan", $id);
		$control_tunjangan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tunjangan - end
//	processing insentif - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_insentif = $pageObject->getControl("insentif", $id);
		$control_insentif->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing insentif - end
//	processing bonus - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_bonus = $pageObject->getControl("bonus", $id);
		$control_bonus->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing bonus - end
//	processing thr - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_thr = $pageObject->getControl("thr", $id);
		$control_thr->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing thr - end
//	processing pajak - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_pajak = $pageObject->getControl("pajak", $id);
		$control_pajak->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pajak - end
//	processing pinjaman - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_pinjaman = $pageObject->getControl("pinjaman", $id);
		$control_pinjaman->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pinjaman - end
//	processing gaji_bersih - begin
	$condition = 1;

	if($condition)
	{
		$control_gaji_bersih = $pageObject->getControl("gaji_bersih", $id);
		$control_gaji_bersih->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing tanggal_bayar - begin
	$condition = 1;

	if($condition)
	{
		$control_tanggal_bayar = $pageObject->getControl("tanggal_bayar", $id);
		$control_tanggal_bayar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tanggal_bayar - end
//	processing tanggal_transfer - begin
	$condition = 1;

	if($condition)
	{
		$control_tanggal_transfer = $pageObject->getControl("tanggal_transfer", $id);
		$control_tanggal_transfer->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tanggal_transfer - end
//	processing nama_bank - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_nama_bank = $pageObject->getControl("nama_bank", $id);
		$control_nama_bank->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nama_bank - end
//	processing nama_rekening - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_nama_rekening = $pageObject->getControl("nama_rekening", $id);
		$control_nama_rekening->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nama_rekening - end
//	processing no_rekening - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_no_rekening = $pageObject->getControl("no_rekening", $id);
		$control_no_rekening->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing no_rekening - end
//	processing sk_penghasilan - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sk_penghasilan = $pageObject->getControl("sk_penghasilan", $id);
		$control_sk_penghasilan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_penghasilan"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: penghasilan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["gaji_pokok"] = $evalues["gaji_pokok"];
	$data["tunjangan"] = $evalues["tunjangan"];
	$data["insentif"] = $evalues["insentif"];
	$data["bonus"] = $evalues["bonus"];
	$data["thr"] = $evalues["thr"];
	$data["pajak"] = $evalues["pajak"];
	$data["pinjaman"] = $evalues["pinjaman"];
	$data["gaji_bersih"] = $evalues["gaji_bersih"];
	$data["cara_bayar"] = $evalues["cara_bayar"];
	$data["tanggal_bayar"] = $evalues["tanggal_bayar"];
	$data["tanggal_transfer"] = $evalues["tanggal_transfer"];
	$data["nama_bank"] = $evalues["nama_bank"];
	$data["nama_rekening"] = $evalues["nama_rekening"];
	$data["no_rekening"] = $evalues["no_rekening"];
	$data["sk_penghasilan"] = $evalues["sk_penghasilan"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("gaji_pokok_label","<label for=\"".GetInputElementId("gaji_pokok", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tunjangan_label","<label for=\"".GetInputElementId("tunjangan", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("insentif_label","<label for=\"".GetInputElementId("insentif", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("bonus_label","<label for=\"".GetInputElementId("bonus", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("thr_label","<label for=\"".GetInputElementId("thr", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pajak_label","<label for=\"".GetInputElementId("pajak", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pinjaman_label","<label for=\"".GetInputElementId("pinjaman", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("gaji_bersih_label","<label for=\"".GetInputElementId("gaji_bersih", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tanggal_bayar_label","<label for=\"".GetInputElementId("tanggal_bayar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tanggal_transfer_label","<label for=\"".GetInputElementId("tanggal_transfer", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nama_bank_label","<label for=\"".GetInputElementId("nama_bank", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nama_rekening_label","<label for=\"".GetInputElementId("nama_rekening", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("no_rekening_label","<label for=\"".GetInputElementId("no_rekening", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sk_penghasilan_label","<label for=\"".GetInputElementId("sk_penghasilan", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='penghasilan_view.php?".$viewlink."'\"");
